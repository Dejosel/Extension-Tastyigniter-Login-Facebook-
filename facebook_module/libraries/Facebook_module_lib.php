...
class Facebook_module_lib {
    ...
    public function __construct() {
        // Load settings
        $facebook_m = $this->extension->getModule('facebook_module');
        $settings = !empty($facebook_m['ext_data']) ? $facebook_m['ext_data'] : array();
        ...
        if (!isset($this->fb)){
            $this->fb = new FB([
                ...
                'default_graph_version' => $settings['facebook_graph_version']
            ]);
        }
        // Load correct helper depending on login type
        // set in the config file
        switch ($settings['facebook_login_type']){
            case 'js':
        ...
        if ($settings['facebook_auth_on_load'] === '1'){
    ...
    public function login_url(){
        ...
        if($settings['facebook_login_type'] != 'web'){
            return '';
        }
        // Get login url
        return $this->helper->getLoginUrl(
            base_url() . $settings['facebook_login_redirect_url'],
            $settings['facebook_permissions']
        );
    }
    ...
    public function logout_url(){
        ...
        return $this->helper->getLogoutUrl(
            ...
            base_url() . $settings['facebook_logout_redirect_url']
        );
    }
